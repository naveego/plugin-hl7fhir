...
using PluginHL7FHIR.API.Discover;
using PluginHL7FHIR.API.Factory;
using PluginHL7FHIR.API.Read;
using PluginHL7FHIR.API.Write;
using PluginHL7FHIR.DataContracts;
using PluginHL7FHIR.Helper;
using Task = System.Threading.Tasks.Task;

namespace PluginHL7FHIR.Plugin
{
    public class Plugin : Publisher.PublisherBase
    {
        ...
        public Plugin(FhirClient client = null)
        {
            _apiClientFactory = new ApiClientFactory(client);
        ...
        public override async Task<ConnectResponse> Connect(ConnectRequest request, ServerCallContext context)
        {
            ...
            try
            {
                _apiClient = _apiClientFactory.CreateApiClient(settings);

                var client = _apiClient.GetClient();
                
                var capabilityStatement = await client.CapabilityStatementAsync();

                if (capabilityStatement.Status != PublicationStatus.Active)
                {
                    var e = new Exception($"Client created with {settings.Endpoint} did not find an active database.");
                    Logger.Error(e, e.Message);
